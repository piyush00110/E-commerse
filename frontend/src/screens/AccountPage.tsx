    ...
    <div style={{ maxWidth: 1000, margin: '0 auto', padding: '32px 24px' }}>
        ...
        <div style={{ background: 'white', borderRadius: 12, padding: 24, boxShadow: '0 2px 8px rgba(0,0,0,0.08)' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
            ...
            <button onClick={() => setEditing(!editing)}
              style={{ padding: '6px 14px', border: '1px solid #ddd', borderRadius: 6, background: 'white', fontSize: 13, cursor: 'pointer' }}>
              {editing ? 'Cancel' : 'Edit'}
            </button>
          </div>

          {saved && <div style={{ color: '#067d62', marginBottom: 12, fontSize: 14 }}>{'\u2713'} Profile updated!</div>}
        ...
        <div style={{ background: 'white', borderRadius: 12, padding: 24, boxShadow: '0 2px 8px rgba(0,0,0,0.08)' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            <Link to="/orders" style={{
              ...
              border: '1px solid #b0c4de', color: '#232f3e', fontWeight: 600, fontSize: 14, textDecoration: 'none',
            }}>
              {'\u{1F4E6}'} My Orders ({orders.length})
            </Link>
            <Link to="/wishlist" style={{
              padding: '14px 20px', background: '#fff0f5', borderRadius: 8,
              border: '1px solid #ffb6c1', color: '#8a0040', fontWeight: 600, fontSize: 14, textDecoration: 'none',
            }}>
              {'\u2764'} My Wishlist
            </Link>
            <Link to="/cart" style={{
              padding: '14px 20px', background: '#fff8f0', borderRadius: 8,
              border: '1px solid #ffd700', color: '#8a5100', fontWeight: 600, fontSize: 14, textDecoration: 'none',
            }}>
      ...
      <div style={{ background: 'white', borderRadius: 12, padding: 24, boxShadow: '0 2px 8px rgba(0,0,0,0.08)', marginTop: 24 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 16, flexWrap: 'wrap' }}>
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: 32, fontWeight: 800, color: '#b12704' }}>
              ${(() => { try { return Number(localStorage.getItem('walletBalance')) || 0; } catch { return 0; } })().toFixed(2)}
            </div>
            <div style={{ fontSize: 13, color: '#565959' }}>Available balance</div>
          </div>
      ...
      <div style={{ background: 'white', borderRadius: 12, padding: 24, boxShadow: '0 2px 8px rgba(0,0,0,0.08)', marginTop: 24 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
          ...
          <button onClick={() => { setShowAddrForm(!showAddrForm); setEditAddrId(null); setAddrForm({ street: '', city: '', state: '', zip: '', phone: '' }); }}
            style={{ padding: '6px 14px', background: '#232f3e', color: 'white', border: 'none', borderRadius: 6, fontSize: 13, fontWeight: 600, cursor: 'pointer' }}>
            + Add Address
          </button>
        </div>
          ...
          <div style={{ background: '#f8f9fa', borderRadius: 8, padding: 20, marginBottom: 16 }}>
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              ...
              <button onClick={() => { setShowAddrForm(false); setEditAddrId(null); }}
                style={{ padding: '10px 20px', border: '1px solid #ddd', borderRadius: 8, background: 'white', fontSize: 14, cursor: 'pointer' }}>
                Cancel
              </button>
            </div>
          </div>
        ...
        {addresses.length === 0 && !showAddrForm ? (
          <p style={{ color: '#565959', textAlign: 'center', padding: 24 }}>No saved addresses. Add one for faster checkout!</p>
        ) : (
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: 12 }}>
            {addresses.map((addr) => (
              <div key={addr.id} style={{
                border: '1px solid #ddd', borderRadius: 8, padding: 16, position: 'relative',
              }}>
                <div style={{ fontWeight: 600, marginBottom: 4 }}>{addr.label}</div>
                <div style={{ fontSize: 13, color: '#565959', lineHeight: 1.5 }}>
                  <div>{addr.street}</div>
                ...
                <div style={{ marginTop: 8, display: 'flex', gap: 12 }}>
                  ...
                  <button onClick={() => handleDeleteAddress(addr.id)}
                    style={{ background: 'none', border: 'none', color: '#b12704', fontSize: 13, cursor: 'pointer', padding: 0 }}>
                    Remove
                  </button>
                </div>
              </div>
      ...
      <div style={{ background: 'white', borderRadius: 12, padding: 24, boxShadow: '0 2px 8px rgba(0,0,0,0.08)', marginTop: 24 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
          ...
          {orders.length > 0 && <Link to="/orders" style={{ color: '#007185', fontSize: 13 }}>View all {'\u2192'}</Link>}
        </div>
        {orders.length === 0 ? (
          <p style={{ color: '#565959', textAlign: 'center', padding: 24 }}>No orders yet. <Link to="/products" style={{ color: '#007185' }}>Start shopping!</Link></p>
        ) : (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {orders.slice(0, 3).map((order) => (
              <div key={order._id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px 0', borderBottom: '1px solid #eee' }}>
                <div>
                  <div style={{ fontFamily: 'monospace', fontSize: 13, fontWeight: 600 }}>#{order._id.slice(-8).toUpperCase()}</div>
                  <div style={{ fontSize: 13, color: '#565959' }}>{new Date(order.createdAt).toLocaleDateString()} - {order.items?.length} items</div>
                </div>
