...
const OrderConfirmationPage: React.FC = () => {
  ...
  return (
    <div style={{ maxWidth: 800, margin: '0 auto', padding: '32px 24px' }}>
      <div style={{ textAlign: 'center', background: 'white', borderRadius: 16, padding: '48px 24px', boxShadow: '0 2px 12px rgba(0,0,0,0.08)', marginBottom: 24 }}>
        <div style={{
          width: 80, height: 80, borderRadius: '50%', background: '#d4edda', color: '#155724',
        ...
        <p style={{ color: '#565959', fontSize: 16, marginBottom: 4 }}>
        ...
        <p style={{ color: '#067d62', fontSize: 14, fontWeight: 500, marginTop: 12 }}>
      ...
      <div style={{ background: 'white', borderRadius: 12, padding: 24, boxShadow: '0 2px 8px rgba(0,0,0,0.08)', marginBottom: 16 }}>
        ...
        {order.items.map((item, idx) => (
          <div key={idx} style={{ display: 'flex', gap: 12, alignItems: 'center', padding: '10px 0', borderBottom: idx < order.items.length - 1 ? '1px solid #eee' : 'none' }}>
            <img src={item.image} alt="" style={{ width: 60, height: 60, borderRadius: 8, objectFit: 'contain', background: '#f8f8f8' }} />
            <div style={{ flex: 1 }}>
              <Link to={`/products/${item.product}`} style={{ color: '#007185', fontSize: 14, fontWeight: 500 }}>{item.name}</Link>
      ...
      <div style={{ background: 'white', borderRadius: 12, padding: 24, boxShadow: '0 2px 8px rgba(0,0,0,0.08)', marginBottom: 24 }}>
        ...
        <div className="summary-row"><span>Shipping</span><span style={{ color: order.shippingPrice === 0 ? '#067d62' : 'inherit', fontWeight: order.shippingPrice === 0 ? 600 : 400 }}>{order.shippingPrice === 0 ? 'FREE' : `$${order.shippingPrice.toFixed(2)}`}</span></div>
      ...
      <div style={{ display: 'flex', gap: 16, justifyContent: 'center', flexWrap: 'wrap' }}>
        <Link to="/orders" style={{
          padding: '12px 32px', background: '#232f3e', color: 'white', borderRadius: 8, fontWeight: 600, fontSize: 15, textDecoration: 'none',
        }}>
          View All Orders
        </Link>
        <Link to="/products" style={{
          padding: '12px 32px', background: '#ff9900', color: '#232f3e', borderRadius: 8, fontWeight: 600, fontSize: 15, textDecoration: 'none',
        }}>
          Continue Shopping
        </Link>
      </div>
